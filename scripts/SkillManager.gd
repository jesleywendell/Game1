...
func _on_skill_q_body_entered(body: Node) -> void:
	if skill_q_hit_targets.has(body):
		return
	skill_q_hit_targets.append(body)
	var dir := Vector2.ZERO
	if _player_ref:
		dir = (body.global_position - _player_ref.global_position).normalized()
	if body.has_method("take_damage"):
		body.take_damage(DAMAGE_Q, dir)
	elif body.has_method("receive_hit"):
		body.receive_hit(DAMAGE_Q, dir)
